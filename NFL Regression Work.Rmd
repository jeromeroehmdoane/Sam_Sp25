...
get_results_matrix <- function(season_wanted, week_wanted, game_dataset){       #week means including that
  ...
  all_teams <- sort(unique(c(prior_games$away_team, prior_games$home_team)))
...
get_results_matrix(2023,5,games)
```
...
get_pagerank <- function(season_wanted, week_wanted, game_dataset, show_graph = FALSE){
  if(week_wanted == 0){
    all_teams <- sort(unique(c(game_dataset$away_team, game_dataset$home_team)))
...
get_pagerank(2023,1,games)

```

```{r}
teams_matrix <- get_results_matrix(2023, 5, games)
graph_adj_matrix <- graph_from_adjacency_matrix(t(data.matrix(teams_matrix)), weighted = TRUE)
```
...
plot(as.numeric(teams_position[1,]),as.numeric(teams_position[2,]))
```
