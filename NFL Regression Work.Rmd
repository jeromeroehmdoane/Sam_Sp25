...
get_turnovers_created <- function(season_num,
                             week_num,
                             team_name){
  prior_games <- all_fastR_team_stats %>% filter(team==team_name,season==season_num,week<week_num)
  prior_games <- mutate(prior_games,
                  turnovers_created = "NEED VARIABLES HERE",
                  tot_yards = "MAY NOT NEED THIS")
  if(sum(prior_games$plays)>0){
    return(sum(prior_games$tot_yards)/sum(prior_games$plays))
  }
  return(5)
...
get_turovers_givenup <- function(season_num,
                               week_num,
                               team_name){
  prior_games <- all_fastR_team_stats %>% filter(opponent_team==team_name,season==season_num,week<week_num)
  prior_games <- mutate(prior_games,
                  turnovers_givenup = "NEED VARIABLES HERE",
                  tot_yards = "MAY NOT NEED THIS")
  if(sum(prior_games$plays)>0){
    return(sum(prior_games$tot_yards)/sum(prior_games$plays))
  }
...
home_turnovers_created <- c()
home__turnovers_givenup <- c()
away_turnovers_created <- c()
away_turnovers_givenup <- c()

for (i in 1:nrow(games)) {
  home_turnovers_created[i] <- get_turnovers_created(games[i,]$season,
                             games[i,]$week,
                             games[i,]$home_team)
  home_turnovers_givenup[i] <- get_turnovers_givenup(games[i,]$season,
                                 games[i,]$week,
                                 games[i,]$home_team)
  away_turnovers_created[i] <- get_turnovers_created(games[i,]$season,
                             games[i,]$week,
                             games[i,]$away_team)
  away_turnovers_givenup[i] <- get_turnovers_givenup(games[i,]$season,
                                 games[i,]$week,
                                 games[i,]$away_team)
}
games$home_turnovers_created <- home_turnovers_created
games$home_incoming_ypp_a <- home_turovers_givenup
games$away_incoming_ypp <- away_turnovers_created
games$away_incoming_ypp_a <- away_turnovers_givenup

games <- games %>% 
  mutate(home_turnovers_diff = home_turnovers_created - home_turnovers_givenup) %>% 
  mutate(away_turnovers_diff = away_turnovers_created - away_turnovers_givenup) %>%
