...
get_results_matrix(2023,0,final_games)
...
for (i in 1:n_games) {
  ...
  home_last10 <- filter(final_games, home_team==cur_home_team | away_team==cur_home_team)%>%
    tail(10)
  last10_wins_home[i] <- sum(home_last10$winner_team == cur_home_team)
  
  #away team last 10 wins
  cur_away_team = final_games[i, ]$away_team
  away_last10 <- filter(final_games, home_team==cur_away_team | away_team==cur_away_team)%>%
    tail(10)
  last10_wins_away[i] <- sum(away_last10$winner_team == cur_away_team)
...
final_games <- final_games %>%
  mutate(
    ...
    last5_away_pointsscore = last5_away_pointsscore,
